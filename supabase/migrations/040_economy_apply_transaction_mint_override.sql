-- TriplePoker: Rise - economy_apply_transaction: add p_mint_override for cross-currency conversion
-- Run manually in Supabase SQL Editor. Requires 036 to already be applied.
--
-- Central Economy Ledger Phase 7 Round 7 (TOKEN <-> CROWN Conversion). A Token->Crown exchange is
-- correctly modeled as "burn TOKEN, mint CREST" (not a value-preserving TRANSFER) because TOKEN and
-- CREST are fully independent economies with separate Genesis/Mint/Burn counters. The existing BURN
-- branch already lets a caller override the burn-counter bump via p_burn_override, but it never
-- touches cumulative_token_mint/cumulative_crest_mint — so a BURN transaction with a positive CREST
-- entry would correctly credit the player's real Crest balance (the per-entry loop mutates real
-- balances regardless of type) but would silently NOT record that CREST as minted, permanently
-- drifting economy_reconciliation()'s CREST expected_supply below actual_supply on every exchange.
--
-- Fix: add a new optional p_mint_override param (same {token, crest} shape as p_burn_override). When
-- provided on a BURN transaction, it additionally bumps the mint counters -- independent of, and in
-- addition to, the existing burn-override logic. Keeps the whole conversion inside one atomic call
-- (no separate burn()+mint() calls, which would risk losing money if the second call failed after
-- the first succeeded).
...
CREATE OR REPLACE FUNCTION economy_apply_transaction(
  ...
  p_mint_override JSONB DEFAULT NULL  -- {"token": <bigint>, "crest": <bigint>} — BURN type only (cross-currency conversion)
)
...
BEGIN
  ...
  -- entries' own SUM. BURN accepts an explicit p_burn_override because a match-settlement BURN's
  -- entries can legitimately include an untracked-money-returning component (the escrow buy-in)
  -- that must NOT be counted as burn. BURN additionally accepts p_mint_override, independent of
  -- p_burn_override, for the cross-currency-conversion case: a BURN transaction whose entries burn
  -- one currency and mint another (Token->Crown exchange) needs BOTH counters bumped correctly in
  ...
  ELSIF p_type = 'BURN' THEN
    ...
    ELSE
      ...
      WHERE id;
    END IF;
    IF p_mint_override IS NOT NULL THEN
      UPDATE economy_global_counters SET
        cumulative_token_mint = cumulative_token_mint + COALESCE((p_mint_override->>'token')::BIGINT, 0),
        cumulative_crest_mint = cumulative_crest_mint + COALESCE((p_mint_override->>'crest')::BIGINT, 0),
        updated_at = NOW()
      WHERE id;
    END IF;
  END IF;
...
DROP FUNCTION IF EXISTS economy_apply_transaction(TEXT, TEXT, TEXT, JSONB, JSONB, JSONB, TEXT, JSONB);

REVOKE ALL ON FUNCTION economy_apply_transaction(TEXT, TEXT, TEXT, JSONB, JSONB, JSONB, TEXT, JSONB, JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION economy_apply_transaction(TEXT, TEXT, TEXT, JSONB, JSONB, JSONB, TEXT, JSONB, JSONB) TO service_role;
