-- Bronze is the guided league: only Silver and later clears count toward a win streak.
...
BEGIN
  ...
  SET tier_d_solo_level = CASE WHEN p_won THEN tier_d_solo_level + 1 ELSE tier_d_solo_level END,
      tier_d_current_win_streak = CASE
        WHEN tier_d_solo_level >= 51 AND p_won THEN tier_d_current_win_streak + 1
        ELSE 0
      END,
      tier_d_best_win_streak = CASE
        WHEN tier_d_solo_level >= 51 AND p_won THEN GREATEST(tier_d_best_win_streak, tier_d_current_win_streak + 1)
        ELSE tier_d_best_win_streak
      END,
  ...
  RETURNING jsonb_build_object('level',tier_d_solo_level,'currentWinStreak',tier_d_current_win_streak,'bestWinStreak',tier_d_best_win_streak,'tierUnlockedMax',tier_unlocked_max) INTO result;
  RETURN result;
END;
$$ LANGUAGE plpgsql;
