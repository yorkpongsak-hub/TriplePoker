...
import { LANGUAGE_NAMES, LANGUAGES } from './translate';
import { useLanguage } from './languageStore';
export default function LanguagePicker() {
  const {language,choice,select,saveError}=useLanguage();
  const [open,setOpen]=useState(false);
  return <>
    <Touch accessibilityRole="button" accessibilityLabel="Choose your language" onPress={()=>setOpen(true)} style={{minHeight:44,justifyContent:'center'}}><Text style={[s.link,{fontSize:13}]}>Language{' · '}{LANGUAGE_NAMES[language]}</Text></Touch>
    <Modal visible={open} transparent animationType="fade" onRequestClose={()=>setOpen(false)}>
      <View style={{flex:1,backgroundColor:'rgba(0,0,0,0.8)',padding:20,alignItems:'center',justifyContent:'center'}}><View style={[s.panel,{width:'100%',maxWidth:420,maxHeight:'90%'}]}>
        ...
        <ScrollView><Touch accessibilityRole="radio" accessibilityState={{checked:choice===null}} onPress={()=>select(null)} style={{paddingVertical:12}}><Text style={s.body}>{choice===null?'✓ ':''}Use device language</Text></Touch>
          {LANGUAGES.map(code=><Touch key={code} accessibilityRole="radio" accessibilityLabel={LANGUAGE_NAMES[code]} accessibilityState={{checked:choice===code}} onPress={()=>select(code)} style={{paddingVertical:12,borderTopWidth:1,borderColor:C.line}}><Text style={{color:choice===code?C.gold:C.text,fontSize:17}}>{choice===code?'✓ ':''}{LANGUAGE_NAMES[code]}</Text></Touch>)}
        </ScrollView>
