...
const aliases:Record<string,string> = {
  'Matches 1 and 2 use the practice view. From match 3, take your seat at the TriplePoker : Rise table with the same rules. You can switch back to practice any time.':'Matches 1 and 2 use the practice view. From match 3, take your seat at the TriplePoker table with the same rules. You can switch back to practice any time.',
...
const legacyLanguage=(language:Language|Locale):Language=>language==='zh-CN'?'zh':language;
export function lookupTranslation(source:string,language:Language|Locale):string|undefined {
  const key=normalize(source),index=LANGUAGES.indexOf(legacyLanguage(language));
...
export function translate(source:string,language:Language|Locale):string {
...
export function translateParts(parts:readonly (string|number)[],language:Language|Locale):string {
