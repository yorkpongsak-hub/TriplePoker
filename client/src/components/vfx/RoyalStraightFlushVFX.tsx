...
export default function RoyalStraightFlushVFX({ playerName = 'PLAYER', onClose, reduceMotionOverride }: Props) {
  ...
  useEffect(() => {
    ...
    if (reduceMotion) { fan.setValue(1); glow.setValue(0.7); return }
    ...
    return () => { loopRef.current?.stop(); backdrop.stopAnimation(); reveal.stopAnimation(); fan.stopAnimation(); glow.stopAnimation() }
  }, [reduceMotion])
  ...
  return <View style={styles.root}>
    ...
    <Pressable accessibilityRole="button" accessibilityLabel="Close Royal Straight Flush celebration" onPress={onClose} style={({ pressed }) => [styles.close, pressed && styles.closePressed]}>
      <Text style={styles.closeText}>CLOSE CELEBRATION</Text>
    </Pressable>
  </View>
}
