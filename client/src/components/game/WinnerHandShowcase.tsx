...
export function WinnerHandShowcase({ pile, centerCards, winnerCards, handRanking, winnerOrigin, isCombo, isSuperCombo, onComplete }: WinnerHandShowcaseProps) {
  ...
  useEffect(() => {
    const centerDuration = rankLeadMs + 550 + Math.max(0, centerCards.length - 1) * 130
    ...
    return () => { clearTimeout(complete); label.stopAnimation() }
  }, [centerCards.length, winnerCards.length, label])
  const winnerDelay = rankLeadMs + 550 + Math.max(0, centerCards.length - 1) * 130
  ...
  return <View pointerEvents="none" style={s.overlay} accessibilityLiveRegion="polite">
    <Animated.View style={[s.rank, winnerOrigin === 'bottom' && s.rankWinner, { opacity: label, transform: [{ scale: label.interpolate({ inputRange: [0, 1], outputRange: [.92, 1] }) }] }]}><Text style={s.rankText}>{t(handRankKey(handRanking), {}, locale)}</Text>{isSuperCombo ? <Text style={s.vfx}>{t('game.superCombo', {}, locale)}</Text> : isCombo ? <Text style={s.vfx}>{t('game.combo', {}, locale)}</Text> : null}</Animated.View>
...
const s = StyleSheet.create({ overlay: { ...StyleSheet.absoluteFill, zIndex: 70, elevation: 70, alignItems: 'center', justifyContent: 'center' }, hand: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', minHeight: 84, transform:[{translateY:-92}], paddingHorizontal:4 }, divider: { width: 1, height: 48, backgroundColor: 'rgba(255,215,106,.75)', marginHorizontal: 7 }, rank: { position:'absolute', top:46, alignItems: 'center', paddingHorizontal:16, paddingVertical:5 }, rankWinner: { borderTopWidth:1, borderBottomWidth:1, borderColor:'rgba(255,215,106,.8)', backgroundColor:'rgba(7,36,20,.72)' }, rankText: { color: '#fff0a8', fontSize: 22, fontWeight: '700', letterSpacing: .5 }, vfx: { color: '#8dffb5', fontSize: 15, fontWeight: '900', marginTop: 6 } })
