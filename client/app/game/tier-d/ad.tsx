...
export default function TierDItemAd(){
 ...
 if(placement==='forced')return <View style={s.screen}><Pressable style={s.claimButton} onPress={forced}><Text style={s.claimText}>CONTINUE</Text></Pressable></View>
 ...
 return <View style={s.screen}>{!message?<View style={s.claimPanel}><Text style={s.title}>REFILL {selected.replaceAll('_',' ').toUpperCase()} ×1</Text><Pressable onPress={finish} style={s.claimButton}><Text style={s.claimText}>WATCH AD</Text></Pressable></View>:<View style={s.claimPanel}>{grantedItem?<><Image source={REWARD_FREE_FX} resizeMode="contain" style={s.rewardFx}/><Text style={s.title}>{message}</Text></>:<Text style={s.title}>{message}</Text>}{!claiming.current&&!grantedItem?<Pressable onPress={finish} style={s.claimButton}><Text style={s.claimText}>RETRY CLAIM</Text></Pressable>:null}</View>}</View>
}
const s=StyleSheet.create({screen:{flex:1,backgroundColor:'#000',alignItems:'center',justifyContent:'center'},claimPanel:{alignItems:'center',gap:18,padding:24},rewardFx:{width:260,height:146},title:{color:'#FFD76A',fontSize:18,fontWeight:'900',textAlign:'center'},claimButton:{paddingHorizontal:22,paddingVertical:14,borderRadius:9,borderWidth:2,borderColor:'#FFD76A',backgroundColor:'#3b2b0c'},claimText:{color:'#FFF2B0',fontWeight:'900',fontSize:14}})
