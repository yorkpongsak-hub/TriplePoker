/**
 * index.tsx — Monarch Encounter (3 real Human + Monarch Boss)
 * Copy จาก game/highNoble/index.tsx (multiplayer) — ต่างกันแค่ tier: 'monarch' ตอน game_join
 * Boss Monarch สลับบุคลิก Four Gods ใหม่ทุก Round (ดู highNobleMultiEngine.ts monarchSchedule)
 * จนกว่าจะได้ชื่อไทย/storyline จริงจากลุงเยาะ — ชื่อ/รูปยังใช้ placeholder "Monarch" (ดู CLAUDE.md pending)
 * The Sage Unicorn Studio Co., Ltd.
 */

import React, { useCallback, useEffect, useRef, useState } from 'react'
import {
  Animated, Image, PanResponder, Platform, ScrollView, StatusBar, StyleSheet,
...
const GameTableLive: React.FC = () => {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      socket.emit('game_join', { roomId: ROOM_ID, userId: PLAYER_ID, tier: 'monarch' })
    })
