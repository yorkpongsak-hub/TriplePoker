...
15. **Grandmaster (Tier S) — Audit 2026-08-07:** state machine/auction/GF/fog-of-war/settlement math/sovereign lifecycle ทั้งหมดสร้างและเทสแยกโมดูลผ่านหมดแล้ว แต่ **"เนื้อเกมจริง" ยังไม่เชื่อมกันเป็นเส้นเดียว 3 จุดใหญ่:**
    - **ไม่มี UI จัดไพ่ลงกอง** — `FanHand.tsx` แค่โชว์ไพ่เป็นพัด ไม่มี drop-zone 3 กอง ผู้เล่นยังลากไพ่ลงกอง 1/2/3 ไม่ได้จริง
    - **ไม่มีการตัดสินผู้ชนะแต่ละกอง** — phase `RESOLVE_PILE_1/2/3` ใน `arenaMatchEngine.ts` แค่ transition เฉยๆ ไม่เคยเรียก `wildHandEvaluator.ts` (มีอยู่แล้ว รองรับ Joker wild) เลย และ action `ARRANGE_1`/`FINAL_ARRANGE`/`FINAL_LOCK` รับ `arrangementHash` เป็น string ทึบที่ server ไม่เคยตรวจสอบ/แปลงกลับเป็นข้อมูลไพ่จริง
    - **Settlement engine ไม่เคยถูกเรียกจาก runtime จริง** — `ArenaSettlementEngine`/`ArenaCrestLedger`/`ArenaSettlementPersistence` สร้างและเทสผ่านแยกโมดูลหมดแล้ว แต่ `ArenaRuntime`/`ArenaMatchEngine` ไม่เคยสร้างหรือเรียกใช้เลย ⇒ ไม่มีการหัก Ante/Auction/Call/Boss Fee หรือจ่ายรางวัลจริงระหว่างเล่น
    - รอบ 2026-08-07 แก้เฉพาะ **gameplay projection** (per-viewer snapshot ส่งไพ่/community card/auction-joker-GF prompt จริงตาม fog of war ผ่าน `arenaProjection.ts` ใหม่, personalized emit ใน `arenaSocket.ts`, บอท/AI ตอบสนองทันทีผ่าน `driveBots()` แทนรอ timeout) — เลข Crest/Crown (pot/seat balance/result modal) ยังเป็น placeholder เหมือนเดิม งานทั้ง 3 จุดข้างต้นเป็น milestone ถัดไปของ Grandmaster
