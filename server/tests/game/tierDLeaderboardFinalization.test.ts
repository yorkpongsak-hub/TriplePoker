...
describe('Tier D League finalization migration', () => {
  ...
  test('only a successful League final-level settlement dispatches finalization', async () => {
    ...
    expect(mockRpc).toHaveBeenCalledWith('finalize_tier_d_league', { p_user_id: 'u1', p_league_id: 'bronze', p_final_level: 50, p_final_points: 40 })
    expect(mockRpc).toHaveBeenCalledWith('grant_tier_d_league_awards', { p_user_id: 'u1', p_league_id: 'bronze' })
  })
})
